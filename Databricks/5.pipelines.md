...
Para que Databricks deduza correctamente o grafo de dependencias do pipeline, o máis natural é referirse ás saídas anteriores polo nome do dataset definido dentro da propia pipeline. O catálogo e o esquema de publicación resólvense a partir da configuración do pipeline.
...
def persoas_silver():
    return (
        spark.read.table("persoas_bronze")
...
def persoas_por_cidade():
    return (
        spark.read.table("persoas_silver")
        .groupBy("cidade")
        .agg(
...
Despois de executar o pipeline, a interface pode mostrar o grafo de dependencias entre as tres saídas mediante o botón **Pipeline graph**. Isto ocorre porque `persoas_silver` se define a partir de `persoas_bronze`, e `persoas_por_cidade` a partir de `persoas_silver`, é dicir, usando referencias internas ao propio pipeline.
...
En definitiva, tanto a versión en Python como a versión en SQL son dúas formas equivalentes de expresar o mesmo pipeline declarativo. Se se usan nomes completamente cualificados como `workspace.demo_ldp.persoas_bronze`, Databricks interprétaos como referencias ao catálogo e pode non representar esas relacións como dependencias internas no grafo do pipeline.
