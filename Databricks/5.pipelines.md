...
Descrición: configuración de pares clave/valor nunha task, empregando parámetros como `catalog`, `schema`, `input_file` e `target_table`.  
Fonte: elaboración propia.
...
Descrición: exemplo dun workflow con varias tasks encadeadas, no que unha tarefa produce información que outra reutiliza na fase seguinte.  
Fonte: elaboración propia.
...
Neste curso interesa sobre todo comprendela a nivel conceptual. Non é necesario dominar toda a configuración de Delta Live Tables para entender o seu papel dentro do ecosistema de Databricks.
...
O laboratorio baséase nos seguintes notebooks:

- `csv_to_bronze.py`
- [bronze_to_silver.py](/c:/repo/bda/03.pipelines/Databricks/notebooks/bronze_to_silver.py)
- [silver_to_gold_country.py](/c:/repo/bda/03.pipelines/Databricks/notebooks/silver_to_gold_country.py)
- [silver_to_gold_month.py](/c:/repo/bda/03.pipelines/Databricks/notebooks/silver_to_gold_month.py)
...
O primeiro notebook do laboratorio é `csv_to_bronze.py`.

Este notebook sería o encargado de:
...
Os parámetros principais serían:
...
O primeiro notebook do laboratorio é [bronze_to_silver.py](/c:/repo/bda/03.pipelines/Databricks/notebooks/bronze_to_silver.py).
...
O segundo notebook é [silver_to_gold_country.py](/c:/repo/bda/03.pipelines/Databricks/notebooks/silver_to_gold_country.py).
...
O terceiro notebook é [silver_to_gold_month.py](/c:/repo/bda/03.pipelines/Databricks/notebooks/silver_to_gold_month.py).
...
1. executar `csv_to_bronze.py`
2. executar `bronze_to_silver.py`
3. executar `silver_to_gold_country.py`
4. executar `silver_to_gold_month.py`
...
Este enfoque é especialmente interesante desde o punto de vista docente, porque mostra que unha mesma saída `silver` pode alimentar varias ramas do pipeline.
...
- `gold-sales-by-country.png`
- contido esperado: vista previa da táboa `workspace.gold.sales_by_country_gold`, mostrando columnas como país, número de pedidos, total de vendas e ticket medio
...
Desde un punto de vista conceptual, o mesmo laboratorio tamén permite entender por que Delta Live Tables resulta útil: as dependencias entre capas e saídas finais xa están bastante claras e poden expresarse de forma máis declarativa nun enfoque máis avanzado.
