...
Como peche do capítulo, pódese propoñer un pequeno laboratorio no que se parta das táboas creadas no capítulo 5:
...
Para alternar entre a vista de notebook e a vista de dashboard, o habitual é empregar o menú `View -> Views`.
...
## 6.12 Laboratorio guiado: notebook dashboard con filtros

Como complemento ao laboratorio anterior, pódese crear un **notebook dashboard** a partir das táboas de vendas do capítulo 5.

Neste caso, a idea non é construír un dashboard independente, senón aproveitar un notebook como soporte único para:

- definir filtros
- calcular indicadores
- crear visualizacións
- engadir unha táboa final de apoio

Esta opción resulta especialmente interesante cando se quere reunir nun mesmo espazo:

- filtros na parte superior
- KPIs nunha zona central
- gráficas ordenadas de máis xeral a máis específica
- unha táboa final de detalle ao remate

As táboas empregadas neste laboratorio serán:

- `workspace.silver.invoices_silver`
- `workspace.gold.sales_by_country_gold`
- `workspace.gold.sales_by_month_gold`

### 6.12.1 Estrutura proposta do notebook

Unha organización razoable do notebook podería ser esta:

1. cela de definición de widgets
2. cela markdown co título do dashboard
3. cela coa visualización de vendas por país
4. cela coa visualización da evolución mensual
5. cela cun KPI global
6. cela cunha táboa final baseada en `silver`

Ao converter este notebook nun dashboard, os widgets aparecerán automaticamente na parte superior.

Para alternar entre a vista de notebook e a vista de dashboard, o habitual é empregar o menú `View -> Views`.

### 6.12.2 Cela 1. Filtros con widgets

O primeiro paso consiste en crear dous filtros:

- un filtro por país
- un filtro por mes de factura

Neste exemplo, os valores dos filtros lense automaticamente das táboas `gold`.

```python
countries = ["All"] + [
    row["country"]
    for row in spark.table("workspace.gold.sales_by_country_gold")
    .select("country")
    .distinct()
    .orderBy("country")
    .collect()
...
dbutils.widgets.dropdown("country", "All", countries, "País")

months = ["All"] + [
    row["invoice_month"]
    for row in spark.table("workspace.gold.sales_by_month_gold")
...
dbutils.widgets.dropdown("invoice_month", "All", months, "Mes de factura")
```

Estes widgets serven como filtros interactivos do notebook dashboard.

### 6.12.3 Cela 2. Título do dashboard

Despois dos filtros pode engadirse unha cela markdown sinxela para presentar o panel.

```markdown
# Dashboard de vendas

Resumo visual das vendas por país e por mes de factura.
```

No notebook dashboard, os widgets seguirán situándose enriba, polo que o título quedará inmediatamente debaixo deles.

### 6.12.4 Cela 3. Visualización de vendas por país

A primeira visualización pode construírse coa táboa:

`workspace.gold.sales_by_country_gold`

Neste caso, ten sentido aplicar só o filtro de país, xa que a táboa xa está agregada por ese campo.
...
A partir desta cela pódese crear unha visualización de tipo `Bar` coa seguinte configuración:
...
Esta visualización cumpre a función de ofrecer unha visión xeral por categorías.

### 6.12.5 Cela 4. Visualización da evolución mensual

A segunda visualización parte da táboa:

`workspace.gold.sales_by_month_gold`

Aquí ten sentido aplicar só o filtro de mes.

```sql
%sql
SELECT *
FROM workspace.gold.sales_by_month_gold
WHERE (:invoice_month = 'All' OR invoice_month = :invoice_month)
ORDER BY invoice_month DESC
LIMIT 12
```

A partir desta cela pode crearse unha visualización de tipo `Line` coa seguinte configuración:
...
Esta gráfica introduce a lectura temporal do dashboard.

### 6.12.6 Cela 5. KPI global

Para un indicador principal, resulta máis adecuado partir da capa `silver`, porque nela se poden combinar ao mesmo tempo os filtros por país e por mes.

```sql
%sql
SELECT ROUND(SUM(total_price), 2) AS total_vendas_global
FROM workspace.silver.invoices_silver
WHERE (:country = 'All' OR Country = :country)
  AND (:invoice_month = 'All' OR date_format(InvoiceDate, 'yyyy-MM') = :invoice_month)
```

A partir desta consulta pode crearse unha visualización de tipo `Counter`.

Esta visualización pode situarse na parte central do dashboard como KPI principal.

### 6.12.7 Cela 6. Táboa final de apoio

O notebook dashboard permite ademais incorporar unha táboa na parte inferior, algo moi útil para completar a lectura visual.

Un exemplo sinxelo sería este:
...
Esta cela pode deixarse como táboa, sen transformala noutro tipo de visualización.

O seu papel é servir como apoio ao resto do dashboard, mostrando rexistros concretos xa transformados.

### 6.12.8 Conversión do notebook en dashboard

Unha vez creadas as celas e as visualizacións, o proceso habitual consiste en:

1. crear os widgets
2. executar o notebook
3. xerar as visualizacións correspondentes a partir das celas SQL
4. abrir a opción de dashboard do notebook
5. organizar os elementos segundo unha orde lóxica

Para alternar entre a vista de notebook e a vista de dashboard, o habitual é empregar o menú `View -> Views`.

Un deseño razoable podería ser:

- filtros na parte superior
- título debaixo dos filtros
- KPI global nunha primeira franxa visual
- gráfica por país
- gráfica temporal por mes
- táboa final de detalle

Ao traballar con widgets, convén ter presente que o notebook debe reexecutarse cando cambian os filtros para refrescar as saídas.

Se o notebook emprega varias celas SQL, o modo máis sinxelo adoita ser executar o notebook completo ao modificar un widget.

### 6.12.9 Comentario final

Este segundo laboratorio mostra unha alternativa complementaria ao dashboard independente.

Mentres que o dashboard independente resulta moi adecuado para presentar visualizacións principais, o notebook dashboard permite:

- combinar filtros e resultados nun mesmo espazo
- engadir unha táboa de apoio ao remate
- conservar un fluxo de traballo máis próximo ao notebook

Por iso, ambas opcións poden ser útiles segundo o tipo de panel que se queira construír.

<img src="images/notebook-dashboard-sales-summary.png" alt="Notebook dashboard final coas visualizacións de vendas e filtros" width="750">

Figura 6.5. Notebook dashboard construído a partir das táboas `silver` e `gold`.  
Fonte: elaboración propia.

---

## 6.13 Relación co seguinte capítulo
